...
wezterm.on("update-right-status", function(window, pane)
	...
	-- 日期時間
	local date =
		space(string.format("%s  %s", wezterm.nerdfonts.md_calendar_clock, wezterm.strftime("%Y-%m-%d %H:%M:%S")))
	...
	launch_menu = {
		{
			...
			label = "Ubuntu (WSL)",
			domain = { DomainName = "WSL:Ubuntu" }, -- ← 這裡改成你剛剛查到的名稱
			cwd = "E:\\御芳",
		},
	},

	default_cwd = "E:\\御芳",
	default_prog = { "wsl.exe", "-d", "FedoraLinux-43" },
}
